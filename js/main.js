  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      ...
      if (isValid) {
        ...
        const subject = encodeURIComponent(`Contacto desde Portafolio - ${nameField.value.trim()}`);
        const body = encodeURIComponent(
          `Nombre: ${nameField.value.trim()}\nCorreo: ${emailField.value.trim()}\n\nMensaje:\n${messageField.value.trim()}`
        );
        window.location.href = `mailto:isabellalopera75@gmail.com?subject=${subject}&body=${body}`;
